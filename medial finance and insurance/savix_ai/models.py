...
class InsuranceScheme(db.Model):
    ...
    rural_urban = db.Column(db.String(20), nullable=True)
    special_category = db.Column(db.String(100), nullable=True) # e.g. CAPF, Ex-serviceman, Government employee

    # Premium
    premium_type = db.Column(db.String(50), nullable=True) # OFFICIAL, INDICATIVE, UNAVAILABLE, NO_BENEFICIARY_PREMIUM
    premium_frequency = db.Column(db.String(50), nullable=True)
    premium_amount = db.Column(db.Float, nullable=True)
    premium_min = db.Column(db.Float, nullable=True)
    premium_max = db.Column(db.Float, nullable=True)
    premium_disclaimer = db.Column(db.Text, nullable=True)

    # Coverage
    coverage_amount = db.Column(db.Float, nullable=True)
    coverage_type = db.Column(db.String(100), nullable=True) # Individual, Family Floater, etc.

    # Features
    waiting_period = db.Column(db.String(100), nullable=True)
    pre_existing_disease_waiting_period = db.Column(db.String(100), nullable=True)
    room_rent_limit = db.Column(db.String(150), nullable=True)
    copay = db.Column(db.String(150), nullable=True)
    deductible = db.Column(db.String(150), nullable=True)

    network_hospitals = db.Column(db.String(150), nullable=True)
    cashless_available = db.Column(db.Boolean, default=False)

    maternity = db.Column(db.Boolean, default=False)
    critical_illness = db.Column(db.Boolean, default=False)
    cancer = db.Column(db.Boolean, default=False)
    accident_cover = db.Column(db.Boolean, default=False)
    hospital_cash = db.Column(db.Boolean, default=False)

    renewal = db.Column(db.String(100), nullable=True)
    lifetime_renewal = db.Column(db.Boolean, default=False)
    tax_benefit = db.Column(db.String(150), nullable=True)
    
    claim_features = db.Column(db.Text, nullable=True)
    major_exclusions = db.Column(db.Text, nullable=True)
    ...
    def to_dict(self):
        return {
            ...
            'premium': {
                ...
                'frequency': self.premium_frequency,
                'amount': self.premium_amount,
                'min': self.premium_min,
                'max': self.premium_max,
                'disclaimer': self.premium_disclaimer,
            },
            'coverage': {
                'amount': self.coverage_amount,
                'type': self.coverage_type,
            },
            'features': {
                'waiting_period': self.waiting_period,
                'pre_existing_waiting': self.pre_existing_disease_waiting_period,
                'room_rent_limit': self.room_rent_limit,
                'copay': self.copay,
                'deductible': self.deductible,
                'network_hospitals': self.network_hospitals,
                'cashless_available': self.cashless_available,
                'maternity': self.maternity,
                'critical_illness': self.critical_illness,
                'cancer': self.cancer,
                'accident_cover': self.accident_cover,
                'hospital_cash': self.hospital_cash,
                'renewal': self.renewal,
                'lifetime_renewal': self.lifetime_renewal,
